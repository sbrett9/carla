...
namespace road {
  ...
  std::map<road::Lane::LaneType, std::vector<std::unique_ptr<geom::Mesh>>>
    Map::GenerateOrderedMeshWithLaneJunctions( const rpc::OpendriveGenerationParameters& params,
                                               ...
                                               const geom::Vector3D& maxpos) const
  {
    ...
    // Roads first, one mesh per lane, ordered along each road so consecutive pieces meet.
    const std::vector<RoadId> RoadsToGenerate = FilterRoadsByPosition(minpos, maxpos);
    for (const RoadId Id : RoadsToGenerate) {
      const auto& road = _data.GetRoads().at(Id);
      if (road.IsJunction()) {
        // The roads inside a junction are meshed below, together, so that the surface they form is
        // continuous across the junction rather than a set of overlapping turning paths.
        continue;
      }
      mesh_factory.GenerateAllOrderedWithMaxLen(road, out_mesh_list);
    }

    // Then the junctions, from the lanes that actually cross them. This is deliberately the same
    // construction GenerateChunkedMesh uses: the surface the simulation drives on and the surface
    // saved into a level have to be the same shape, and a junction rebuilt from a distance field is
    // not that shape.
    const std::vector<JuncId> JunctionsToGenerate = FilterJunctionsByPosition(minpos, maxpos);
    for (const JuncId Id : JunctionsToGenerate) {
      ...
      if (params.smooth_junctions) {
        ...
        if (merged_mesh && merged_mesh->GetVertices().size()) {

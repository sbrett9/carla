...
|---|---|---|
| **Bare-earth heights read the mirrored grid row** | The grid is built in the CARLA frame (`DrapeTerrain.cs:56-67`) where `CARLA y = -north` (`Geodesy.cs:107`), and `SumoCotBridge.py` fed SUMO's northing straight in. Independent of the extents: sampling the `.xodr` elevation profile every 5 m, the residual against the grid is **13.04 m** stdev under SUMO-y indexing and **0.63 m** under CARLA-y over 8,132 Arapahoe points | **Fixed at source.** Median 4.42 m, mean 8.53 m, max 40.48 m across 905,017 recorded rows; 89.4% over a metre. The four affected datasets under `Build/telemetry/` were **discarded rather than corrected** — they were month-old test output nobody depended on. The defect never reached the C# path, whose `SampleDrapeGroundElevation` indexes correctly; it existed because the Python telemetry tool never goes near CARLA and so never inherited the frame convention |
| **The scenario and its own telemetry disagree about `t = 0`** | 335 of 335 guard trips satisfy `depart == D\u00d786400 + H\u00d73600` with hours in {7,15,23}, asserting local midnight; `SumoCotBridge.py:197` is UTC-only and stamps `t = 0` from the wall clock at launch \u2014 measured `epoch="2026-08-28T18:00:00.000Z"` and `"2026-08-27T18:00:00.000Z"` on the two shipped datasets | The epoch convention is perfectly consistent and reaches nothing that can read it |
| **Nothing separates the authored from the ambient population where a model reads it — because no artifact a model reads exists yet** | The written CoT XML and CSV are the truth sidecar and carry `type_id`, `special_type`, `role_name` and `marked` by design. vType colour never reaches a blueprint, so a planted vehicle's authoring colour is not in the imagery. The datagram feed is a moving-map display | **Not a defect, and not a clearance.** The defence has nothing to defend today because [`08`](08_Collection_And_EPoL.md)'s training export is unbuilt. It becomes live the moment that export exists, and everything the corpus check measures — which fields separate the two groups — is what it will be measured against then |
| **The PNG metadata chunks carry run configuration** | `carla:solar` embeds `advancing`/`rate`; `carla:capture` embeds `seed`, and `scenario_id` was dormant because nothing passed it. `carla:sensor` embeds the platform's position, pose and intrinsics | **Not a defect.** A recorded still is a truth artifact that detect-and-track validation reads, and a harness needs the scenario, the seed and the sun policy that produced a frame. `carla:sensor` independently passes observer-derivability — the platform's position is the navigation solution and the intrinsics are the sensor. Whether a **training export's** copy of a still may carry run configuration is a live question, and belongs with the export it describes ([`08`](08_Collection_And_EPoL.md) §9.4, stage K) |
...
| **Smaller, same family** | `anomaly_notes` is never read; `scenario_id` never supplied \u2014 and supplying it **activates** a dormant PNG leak, so the two must land together; `FrameRecorder.Dropped` has no reader and the clock ratio is computed then only logged (`SumoCotBridge.py:291`); `sensor_tick` is set nowhere, so every frame renders and the recorder discards **9 in 10** at the shipped defaults (20 Hz tick, 2 Hz record) after render, encode and transport; the bare-earth reader boxes 7.6 M floats (60.9 MB \u2192 243.6 MB, 0.183 s to load) | Each silent |

...
|---|---|---|
| **Bare-earth heights read the mirrored grid row** | The grid is built in the CARLA frame (`DrapeTerrain.cs:56-67`) where `CARLA y = -north` (`Geodesy.cs:107`), and `SumoCotBridge.py:311` feeds SUMO's northing straight in. Independent of the extents: sampling the `.xodr` elevation profile every 5 m, the residual against the grid is **13.05 m** stdev under SUMO-y indexing and **0.615 m** under CARLA-y across 9,474 Arapahoe points, collapsing to a near-constant road-deck offset | Median **4.42 m**, mean **8.53 m**, max **40.48 m** across 905,017 Arapahoe rows; **89.4%** over a metre. **Forward-patchable**: the CSVs carry `sumo_x`/`sumo_y`, the producing grids reproduce the recorded values to 0.0024 m, so the `hae` column is rewritten rather than re-simulated. Exactly four artifacts are affected, all under `Build/telemetry/`; the `SCTMV_recordings` captures come through the C# path and are in the correct frame |

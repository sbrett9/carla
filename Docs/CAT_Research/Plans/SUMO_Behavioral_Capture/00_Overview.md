...
| **Five ground-truth leaks put the answer inside the label channel** | `special_type="marked"`, duplicated by a second `marked` column; anomaly affiliation `u` readable off the CoT type; conspicuous anomaly colours; and the **vType id itself**, written verbatim into `<_carla type_id=\u2026>` and named `anomaly_probe`, `anomaly_escort`, `anomaly_shadow`, `anomaly_staybehind` | A corpus built with the current bridge scores models on reading the answer key. **Two of the five are being added by uncommitted work**, and no corpus has been produced yet \u2014 the only defect here with a closing window |
| **Two leaks inside PNG metadata, and a third chunk adjudicated** | `carla:solar` embeds `advancing`/`rate` (live in all 54 shipped PNGs); `carla:capture` embeds `scenario_id`/`seed` (`seed` live, `scenario_id` dormant, and supplying it activates the leak). `carla:sensor` additionally embeds the platform's exact latitude, longitude, height, pose and pinhole intrinsics | The first two are memorisation handles that are neither truth nor observer-derivable, and go. `carla:sensor` **stays**: the platform's position *is* the navigation solution and the intrinsics *are* the sensor, both granted to a fielded observer by name in the observer-derivability rule, so the allow-list names it rather than tolerating it. The validator reads tEXt chunks, not just file trees, and must reject one of today's PNGs unmodified or it has not been tested |

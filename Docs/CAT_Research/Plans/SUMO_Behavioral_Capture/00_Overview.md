...
| **Two leaks inside PNG metadata, and a third chunk unadjudicated** | `carla:solar` embeds `advancing`/`rate` (live in all 54 shipped PNGs); `carla:capture` embeds `scenario_id`/`seed` (`seed` live, `scenario_id` dormant). `carla:sensor` additionally embeds the platform's exact latitude, longitude, height, pose and pinhole intrinsics | Memorisation handles that are neither truth nor observer-derivable. The anti-leak validator must read tEXt chunks, not just file trees, and must reject one of today's PNGs unmodified or it has not been tested. Whether `carla:sensor` is legitimate \u2014 a real EO consumer knows its own platform telemetry \u2014 is decided in [`08`](08_Collection_And_EPoL.md), not by omission |

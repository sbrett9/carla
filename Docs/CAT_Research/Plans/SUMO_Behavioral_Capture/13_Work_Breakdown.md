...
| ⚑ | **Let the camera's own tiles arrive before the first frame is written. Open — no mechanism exists.** The capture must observe that the view has stopped improving, not wait for a count the caller has to guess | Cesium selects tiles on the world tick from the views registered for it, and a CARLA camera sensor is registered by `ACesiumSensorViewPublisher`, so the selection follows the sensor's frustum. Measured at four cold camera poses: with no pre-roll the first written frame is an empty sky and the imagery takes two to three frames to fill in; at three further poses 120 to 400 ticks of pre-roll produced a first frame indistinguishable from the sixth. **A timed wait is the wrong shape** — nothing renders and nothing streams between ticks in a synchronous world. **A fixed tick count is also the wrong shape**: its right value depends on the view, the cache and the ground, and a coarse level of detail still refining reads as settled to a count. `run_SCTMV.py` never meets this because recording is bound to a key and an operator presses it once the view looks right; an unattended capture has nobody to wait (`03` §9.5.1) |

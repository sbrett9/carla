...
field. Decisions are numbered D6.1 to D6.36 and are stable — sibling documents cite them.
...
| The vocabulary that defines the terms: the core, and every author namespace with its definitions, `broader` links, `contrast_with`, counterfactuals and `hard_negative_for` (§8.7) | **yes — it is what makes the label readable.** It holds no per-subject truth: it says what a term means, never which vehicle carries it | yes |
